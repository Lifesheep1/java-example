...
public class MyStorage {
    ...
    static void prod_input(Scanner s){
        ...
        input = s.nextLine();
        //Ammount를  products 배열인덱스안에 넣어주고싶음
        // 어떻게 넣을것인가??
        System.out.println("[System] 등록이 완료됬습니다.");
        System.out.println("[System] 현재 등록된 제품 목록 ▼");
        for(int i = 0; i < products.length; ++i){ //5번 도는거 반복문
            if(products[i].equals("등록 가능")){ // products의 배열인덱스가 "등록 가능"이면
                products[i] = input;
    ...
    public static void prod_amount_decrease(Scanner s) {
        ...
        input = s.nextLine();
        for (int i = 0; i < products.length; ++i) {
            if (input.equals(products[i])) { // input이 product[i]랑 같을경우
                System.out.print("[System] 원하는 출고량을 입력하세요 :");
                try {
                    cnt = Integer.parseInt(s.nextLine()); // 출고량 입력 받기
                    if(counts[i] - cnt< 0){
                        System.out.println("출고량이 초과되었을 때 수량이 변경되지 않아야 합니다.");
                        break;
                    }
